...
- **`widgets.py` / `ApprovalScreen(ModalScreen[list[dict]])`** — the type parameter says what the
  screen _returns_: the decisions list. Keys (`y`/`n`/`a`/`esc`) and buttons share the same actions.
...
One pause carries **every** gated call from that model turn. Ask for _"write a file and run echo
hi"_ and you get one interrupt with two `action_requests` — the modal shows both, and `dismiss()`
...
4. Note what you still can't see: _what_ the file will contain. That's Part 16's diff view.
...
2. Add a "reject with a reason" input so the model learns _why_ you said no.
