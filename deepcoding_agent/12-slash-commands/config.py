...
# Tools that must be approved by a human before they run. Everything else
# (ls, read_file, glob, grep) is read-only and runs freely.
GATED_TOOLS = ("execute", "write_file", "edit_file", "delete")

# Set DEEPCODER_AUTO_APPROVE=1 to skip every prompt. Handy for demos and
# scripted runs; a bad idea anywhere near a repo you care about.
AUTO_APPROVE = os.getenv("DEEPCODER_AUTO_APPROVE", "").lower() in ("1", "true", "yes")
