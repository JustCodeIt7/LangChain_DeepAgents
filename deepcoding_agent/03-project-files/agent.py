...
def build_backend() -> CompositeBackend:
    ...
    CompositeBackend picks a backend per path prefix, longest match wins:
      ...
      everything else -> FilesystemBackend, real files under WORKDIR
    ...
    return CompositeBackend(
        default=FilesystemBackend(root_dir=str(config.WORKDIR)),
        routes={"/scratch/": StateBackend()},
    )


def build_agent():
    """Assemble the full agent: model + real filesystem + memory."""
