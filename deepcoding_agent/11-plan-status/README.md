...
- **`TodoListMiddleware`** — in deepagents 0.7 the `write_todos` tool is _opt-in_. Without this
...
- **The status bar is a timer, not an event** — `set_interval(1/2, refresh_status)` repaints
  model, workdir, elapsed, and cumulative tokens. Elapsed time ticks _while_ the agent works
