...
class DeepCoderApp(App):
    ...
    def __init__(self, agent=None, thread_id: str = "tui", extra_tools=None) -> None:
        super().__init__()
        self.agent = agent if agent is not None else build_agent(extra_tools)
    ...
    def on_mount(self) -> None:
        ...
        # Say what is wrong before the first turn fails confusingly.
        problem = health.check()
        if problem:
            self.add(f"⚠️  {problem}", "assistant")
    ...
    def action_clear(self) -> None:
        ...
        self.commands.run("/clear")

    def action_new(self) -> None:
        """ctrl+n — start a new conversation."""
        self.add(self.commands.run("/new"), "tool")

    def action_resume(self) -> None:
        """ctrl+r — open the resume picker."""
        self.pick_thread()

    def action_help(self) -> None:
        """f1 — list the commands."""
        self.add(self.commands.run("/help"), "tool")
