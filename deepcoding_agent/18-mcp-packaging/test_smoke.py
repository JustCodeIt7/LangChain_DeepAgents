...
@pytest.mark.anyio
async def test_health_warning_shows_for_a_missing_model(monkeypatch) -> None:
    """A bad model must produce a readable warning, not a traceback."""
    import health

    monkeypatch.setattr(health, "check", lambda: "**Model `nope` is not pulled.**")
    app = DeepCoderApp(agent=PlanningAgent())
    async with app.run_test() as pilot:
        await pilot.pause()
        assert any("not pulled" in line for line in rendered(app))


@pytest.mark.anyio
async def test_keybindings_are_wired() -> None:
    """ctrl+n starts a new thread; f1 lists commands."""
    app = DeepCoderApp(agent=PlanningAgent())
    async with app.run_test() as pilot:
        before = app.thread_id
        await pilot.press("ctrl+n")
        await pilot.pause()
        assert app.thread_id != before

        await pilot.press("f1")
        await pilot.pause()
        assert any("/resume" in line for line in rendered(app))
