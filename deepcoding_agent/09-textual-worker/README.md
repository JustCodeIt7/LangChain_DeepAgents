...
- **Lazy answer widget** — created on the first token, so tool lines that arrive first appear
  _above_ the reply instead of below it.
- **Esc to cancel** — `worker.cancel()`, plus an `is_cancelled` check between events.

## Two real bugs this part taught me

**1. Racing `update()` drops tokens.** `Markdown.update()` returns an _awaitable_ that re-parses
...
**2. Cancellation is cooperative.** `worker.is_cancelled` is checked _between_ events, so Esc can't
