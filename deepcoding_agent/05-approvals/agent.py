...
def build_agent():
    """Assemble the full agent: model + shell + memory + approval gates.

    interrupt_on maps a tool name to True (pause before every call) or to a
    config dict such as {"allowed_decisions": ["approve", "reject"]}.

    The checkpointer is REQUIRED here, not optional: pausing means saving the
    half-finished run somewhere and picking it up after the human answers.
    """
