...
- **`interrupt_on={tool: True}`** — pauses the graph _before_ a gated tool executes. We gate
...
model decides to write a file _and_ run the tests in the same turn, you get **one** interrupt with
...
render this as a _list_ for the same reason.
...
3. Force the batched case: _"In one turn, write a file AND run a command."_ Then explain the list.
