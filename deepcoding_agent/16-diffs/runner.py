...
@dataclass
class ToolStart:
    """The agent began calling a tool. nested=True means inside a subagent."""
...
def _tool_starts(update: Any, seen: set[str], nested: bool) -> Iterator[ToolStart]:
    ...
    for message in update.get("messages", []) or []:
        for call in getattr(message, "tool_calls", []) or []:
            ...
            seen.add(call_id)
            yield ToolStart(
                name=call.get("name", "?"), args=call.get("args", {}) or {}, nested=nested
            )
